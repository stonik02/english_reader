...
Все существующие и новые аккаунты получают роль `user`. Такой пользователь
может удалить только EPUB, который загрузил сам, и может создать до десяти
книг в общем каталоге. Роль `admin` может удалить любую книгу и не ограничена
этим лимитом. Проверка выполняется сервером и не зависит от кнопок в браузере.

Первого администратора назначает оператор с доступом к `DATABASE_URL`. Сначала
убедитесь, что email принадлежит нужному человеку, затем выполните:

```bash
cd backend
make grant-admin ADMIN_EMAIL='family@example.com' CONFIRM=true
```

Команда намеренно не имеет публичного API и требует явного `CONFIRM=true`.
Не передавайте пароль, JWT или URL базы данных в аргументах команды.
