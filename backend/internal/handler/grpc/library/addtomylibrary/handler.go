...
func (h *Handler) AddToMyLibrary(ctx context.Context, request *readerv1.AddToMyLibraryRequest) (*readerv1.UserBook, error) {
	userID, err := h.tokens.Parse(request.GetAccessToken())
	if err != nil {
		return nil, status.Error(codes.Unauthenticated, "invalid access token")
	}
	userBook, err := h.usecase.Execute(ctx, userID, request.GetBookId())
	if err != nil {
		return nil, response.Error(err)
	}

	return response.UserBook(userBook), nil
}
